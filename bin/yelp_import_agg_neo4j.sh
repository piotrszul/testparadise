...
YELP_AGG_DIR="${IMPORT_DIR}/agg-tipped"
...
--nodes:User "${YELP_IMPORT_DIR}/user-header.csv`for i in ${YELP_IMPORT_DIR}/user/*.csv.gz; do echo -n ,$i; done`" \
 ...
 --nodes:Group "${YELP_AGG_DIR}/group-header.csv`for i in ${YELP_AGG_DIR}/group.*/*.csv.gz; do echo -n ,$i; done`" \
 --relationships:PARENT-OF "${YELP_AGG_DIR}/rel-parent_of-header.csv`for i in ${YELP_AGG_DIR}/rel-parent_of.*/*.csv.gz; do echo -n ,$i; done`"\
 --relationships:AG_TIPPED "${YELP_AGG_DIR}/rel-tipped-header.csv`for i in ${YELP_AGG_DIR}/rel-tipped.*/*.csv.gz; do echo -n ,$i; done`"
